...
export function DashboardShell({ productName }: DashboardShellProps) {
    ...
    <main className="rcs-shell">
      <header className="rcs-topbar">
        ...
        <div className="rcs-topbar__meta rcs-topbar__meta--right">
          ...
          <span className="rcs-topbar-chip">{formatPoseChip(robotPose.position.x, robotPose.position.y, robotPose.orientation.yaw)}</span>
          ...
          <div className="rcs-signal" aria-label="MQTT signal status">
            ...
            <span className="rcs-signal__label">{signalRttMs !== null ? `${Math.round(signalRttMs)} ms` : "--"}</span>
          </div>
          <div className="rcs-battery" aria-label="Battery status">
            ...
            <span className="rcs-battery__label">{batteryPercentage !== null ? `${Math.round(batteryPercentage)}%` : DEFAULT_BATTERY_LABEL}</span>
          </div>
        </div>
      </header>
        ...
        <aside className="rcs-sidebar rcs-sidebar--left">
          <section className="rcs-card">
            ...
            <div className="rcs-button-row rcs-button-row--split">
              <button className="rcs-button rcs-button--neutral" type="button" onClick={connectMqtt}>Connect</button>
              <button className="rcs-button rcs-button--teal" type="button" onClick={() => disconnectMqtt()}>Disconnect</button>
            </div>
          </section>
          ...
          <section className="rcs-card">
            <div className="rcs-card__header">
              <h2>Tools</h2>
              <SettingsIconButton label="Open Tools topic settings" onClick={() => setToolsSettingsOpen(true)} />
            </div>
            ...
            <div className="rcs-button-row rcs-button-row--split">
              ...
              <button
                className="rcs-button rcs-button--danger"
                type="button"
                onClick={() => {
            ...
            <button
              className="rcs-button rcs-button--wide rcs-button--teal-soft"
              type="button"
              onClick={() => {
                const x = Number(goalX) || 0;
                const y = Number(goalY) || 0;
          ...
          <section className="rcs-card rcs-card--layers">
            <div className="rcs-card__header">
              <h2>Displays</h2>
              <SettingsIconButton label="Open Displays topic settings" onClick={() => setDisplaySettingsOpen(true)} />
            </div>
        ...
        <aside className="rcs-sidebar rcs-sidebar--right">
          <section className="rcs-card">
            ...
            <div className="rcs-status-list">
              <div className="rcs-status-row"><span>Motion</span><strong>{motionStatus.motion}</strong></div>
              <div className="rcs-status-row"><span>Remaining</span><strong>{formatDistance(motionStatus.remaining_distance ?? null)}</strong></div>
              <div className="rcs-status-row"><span>Heading</span><strong>{formatHeading(motionStatus.heading)}</strong></div>
              <div className="rcs-status-row"><span>Goal</span><strong>{motionStatus.goal_state ?? "Idle"}</strong></div>
              <div className="rcs-status-row"><span>Blocked Source</span><strong>{motionStatus.blocked_source ?? "Clear"}</strong></div>
            </div>
          </section>
      ...
      <TopicSettingsModal
        ...
        title="Tools Topic Settings"
        description="Edit the command topics used by the operator tools and save them locally."
        topics={commandTopicModalEntries}
        values={commandTopics}
        onClose={() => setToolsSettingsOpen(false)}
        onSave={(nextValues) => {
          setCommandTopics(sanitizeTopicRecord(defaultCommandTopics, nextValues));
          setToolsSettingsOpen(false);
          pushEvent("Tools topic settings saved");
        }}
      />

      <TopicSettingsModal
        open={displaySettingsOpen}
        title="Displays Topic Settings"
        description="Edit the viz topics bound to the Displays list and save them locally."
        ...
        onSave={(nextValues) => {
          ...
          pushEvent("Displays topic settings saved");
        }}
      />
    </main>
  );
