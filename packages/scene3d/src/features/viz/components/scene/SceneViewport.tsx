...
function buildTfGroup(
  ...
  const buildAxes = (x: number, y: number, z: number, yaw: number, color?: string) => {
    ...
    const hub = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: color ?? "#8bff8f", depthTest: false, depthWrite: false }),
    );
    hub.renderOrder = 16;
    frameGroup.add(hub);
  ...
  for (const entry of transforms) {
    ...
    const frameGroup = buildAxes(child.x, 0.1, child.y, child.yaw, entry.isStatic ? "#7aa1ff" : "#8bff8f");
    ...
    if (parent) {
      ...
      const line = new THREE.Line(
        lineGeometry,
        new THREE.LineBasicMaterial({
          color: entry.isStatic ? "#5b7dff" : "#56ff86",
          transparent: true,
          opacity: 0.95,
        }),
      );
