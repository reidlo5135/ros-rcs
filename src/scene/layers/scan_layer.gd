...
func _rebuild_scan(scan: Dictionary, robot_pose: Variant) -> void:
	...
	var origin := frame_transform.origin + Vector3(0.0, y_offset, 0.0)
	...
	for index in range(0, points.size(), stride):
		var point := points[index] + Vector3(0.0, y_offset, 0.0)
		mesh.surface_add_vertex(origin)
		mesh.surface_add_vertex(point)
	mesh.surface_end()

	scan_mesh = MeshInstance3D.new()
	scan_mesh.name = "LaserScanRays"
	scan_mesh.mesh = mesh
