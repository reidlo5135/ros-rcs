...
const ScanLayerScript: Script = preload("res://src/scene/layers/scan/scan_layer.gd")
const TfLayerScript: Script = preload("res://src/scene/layers/tf/tf_layer.gd")
...
func _process(delta: float) -> void:
	if camera_mode != "aim" or camera == null:
		return
	var lerp_weight := 1.0 - exp(-8.0 * delta)
	camera_target = camera_target.lerp(last_robot_position, lerp_weight)
	_update_camera()
